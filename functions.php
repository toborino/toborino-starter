...
if ( ! function_exists( 'toborino_setup' ) ) :
	...
	function toborino_setup() {
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'toborino_custom_background_args',
				array(
 ...
 */
function toborino_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'toborino_content_width', 640 );
}
add_action( 'after_setup_theme', 'toborino_content_width', 0 );
 ...
 */
function toborino_scripts() {
	wp_enqueue_style( 'toborino-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'toborino-style', 'rtl', 'replace' );

	wp_enqueue_script( 'toborino-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'toborino_scripts' );
